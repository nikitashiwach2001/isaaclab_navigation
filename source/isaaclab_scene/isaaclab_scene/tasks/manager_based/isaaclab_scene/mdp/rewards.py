...
# ── Temporal lidar diff (multi-step, ego-motion compensated downstream) ──────
...
def navigation_reward_stage4(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    # Goal tether raised to -1.00 for both detour cases: bounds the turn angle so the robot
    # turns just enough to clear the obstacle/wall, not 70-90°. static_wall_detour was -0.20
    # (near-zero tether) which let the robot spin in place near static walls — same over-rotation
    # bug as the dynamic case, fixed the same way.
    r_yaw = torch.where(
    ...
    # 4. Obstacle penalty + closing proximity
    # r_obstacle_soft + r_obstacle_hard match navigation_reward_static (the 100% static-maze reward).
    safe_dist     = 0.65
    terminal_dist = 0.30
...
def navigation_reward_stage4_clean(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    # Yielding suppresses speed-penalty and anti-stuck only when truly trapped/side-threatened.
    # Open front-dodge is NOT yielding: policy should still feel speed/stuck pressure so it doesn't freeze.
    genuinely_trapped = ~((left_min > 0.40) | (right_min > 0.40))
    yielding = (front_dynamic_risk & genuinely_trapped) | left_dynamic_risk | right_dynamic_risk
    ...
    # 4. Obstacle penalty + closing proximity
    # Phase 6: separate static vs dynamic penalty zones based on actual scene geometry.
    # Static (no closing): safe_dist=0.50m, mag=-0.5 — corridors with clearance ≥0.50m are FREE,
    #   narrowest legitimate corridor (inner_wall_6 to outer wall, ~0.39m) costs only -0.16/step.
    # Dynamic (any closing): safe_dist=0.65m, mag=-6.0 — full magnitude moving-threat avoidance, unchanged.
    terminal_dist = 0.30
    ...
    # 7. Clearance recovery
    # Bug-fix #2: also fire when min_obstacle_dist < 0.55 so side-dodge progress gets a clearance reward, not just front.
    clearance_delta = min_obstacle_dist - env.prev_min_obstacle_dist
    env.prev_min_obstacle_dist[:] = min_obstacle_dist
    r_clearance_recovery = torch.where(
